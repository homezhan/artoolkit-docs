...
ARToolKit is well known for the appearance of its [markers][marker_about]: square markers with a black border, and with an arbitrary user-definable image in the interior. The "Hiro" marker used by default by the ARToolKit examples is an iconic example.
...
In general, it is better to use the smallest pattern size possible, at this helps the patterns be recognised better at a greater distance. This table shows the number of barcodes possible with each pattern size:
...
##2D-Barcode Markers as Multimarkers
...
For more information on multimarkers, including their configuration, visit the [multimarker page][marker_multi].

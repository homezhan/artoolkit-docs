...
The ARToolKit NFT 2.0 tracker does not impose any additional constraints. For increased efficiency and robustness, you are allowed to [use feducial markers along with NFT markers as part of the dataset][marker_nft_fiducial_markers], which utilises the NFT 1.0 tracker.

##Producting a Good NFT Image
...
If using a scanner or camera which calculates in terms of image width and height, multiply the resolution calculated by the physical width and height: `width in pixels = width in inches x dots per inch` and  `height in pixels = height in inches x dots per inch`
...
If you have further questions, it would pay to ask questions to the ARToolKit community on the [forum][forum].
